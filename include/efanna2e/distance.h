...
#define AVX512_L2SQR(addr1, addr2, dest, tmp1, tmp2) \
      ...
      dest = _mm512_add_ps(dest, tmp1);
            ...
            int align = 16; //这里改对齐
            if(size % align!= 0) abort();
            __m512 sum_arr[7];
            __m512 l0;
            __m512 r0;

#pragma parallel for reduction(+:result)
            for (unsigned i = 0; i < size; i += align, a += align, b += align) {
                sum_arr[i] = _mm512_setzero_ps();
                AVX512_L2SQR(a+i, b+i, sum_arr[i], l0, r0);
                result += _mm512_reduce_add_ps(sum_arr[i]);
            }
