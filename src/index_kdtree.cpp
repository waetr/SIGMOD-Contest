...
#define _CONTROL_NUM 100
    ...
    void IndexKDtree::meanSplit(std::mt19937 &rng, unsigned *indices, unsigned count, unsigned &index, unsigned &cutdim,
                                float &cutval) {
    ...
    void IndexKDtree::planeSplit(unsigned *indices, unsigned count, unsigned cutdim, float cutval, unsigned &lim1,
                                 unsigned &lim2) {
            ...
            for (; start < end; start++) {
                ...
                for (size_t i = leaf->StartIdx; i < leaf->EndIdx; i++) {
...

                    {
                        ...
                        if (knn_graph[tmpfea].size() < K || dist < knn_graph[tmpfea].top().distance) {
                            Candidate c1(feature_id, dist);
                            knn_graph[tmpfea].push(c1);
                            if (knn_graph[tmpfea].size() > K)
                                knn_graph[tmpfea].pop();
                        }
                    }

                    {
                        LockGuard guard(graph_[feature_id].lock);
                        if (knn_graph[feature_id].size() < K || dist < knn_graph[feature_id].top().distance) {
...
#pragma omp parallel for
        for (unsigned i = 0; i < nd_; i++) {
            ...
            while (!knn_graph[i].empty()) {
                result.insert(knn_graph[i].top().row_id);
                knn_graph[i].pop();
            }
            if (result.size() < K) {
                while (result.size() < K) {
            ...
            final_graph_[i].assign(result.begin(), result.end());
        }
    ...
    void IndexKDtree::Save(const char *filename) {
        ...
        std::cout << "save time: " << diff.count() << std::endl;
    }
